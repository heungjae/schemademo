...
select owner,count(*) cnt from dba_tables where owner in ('SCOTTY', 'HR') group by owner order by 1;
...
select instance_number, instance_name, host_name, version, TO_CHAR(startup_time,'DD-MON-YYYY HH24:MI:SS') startuptime, status, logins, database_status from v$instance;
exit
